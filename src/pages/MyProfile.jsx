  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error: err } = await supabase
        .from('player_profiles')
        .update({ ...form, age, updated_at: new Date().toISOString() })
        .eq('id', profile.id)
      if (err) throw err
    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `Tu es un agent de football professionnel. Génère une description courte et percutante (max 120 mots) pour ce joueur, à la 3ème personne, pour attirer l'attention des recruteurs.

Données: Nom: ${profile.prenom} ${profile.nom}, Poste: ${profile.poste_principal}, Âge: ${profile.age} ans, Club: ${profile.club_actuel}, Région: ${profile.region}, Niveau: ${profile.niveau_championnat}, Catégorie: ${profile.categorie}, Stats: ${profile.matchs_joues} matchs, ${profile.buts} buts, ${profile.passes_decisives} passes décisives, Pied fort: ${profile.pied_fort}, Objectif: ${profile.objectif}

Réponds UNIQUEMENT avec la description, sans titre.`
          }]
        })
      })
      ...
      setForm(f => ({ ...f, ai_description: description }))
  ...
  const StatusBadge = () => {
    const s = {
      en_attente: { label: 'En attente de validation', cls: 'badge-amber' },
          ...
          {editing ? (
            <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
              <div style={S.grid}>
                ...
                <div style={S.fg}><label style={S.label}>Niveau championnat</label><select style={S.input} value={form.niveau_championnat||''} onChange={set('niveau_championnat')}><option value="">Choisir...</option>{NIVEAUX.map(o=><option key={o}>{o}</option>)}</select></div>
              ...
              <label style={{ display:'flex', alignItems:'center', gap:'10px', cursor:'pointer', fontSize:'14px', color:'var(--text2)' }}>
                <input type="checkbox" checked={form.ouvert_opportunites||false}
                  onChange={e => setForm(f => ({ ...f, ouvert_opportunites: e.target.checked }))} />
            ...
            <>
              ...
              {profile.ouvert_opportunites && (
                <div style={{ marginTop:'10px' }}><span className="badge badge-green">✓ Ouvert à d'autres régions / pays</span></div>
              )}
            </>
          )}
        </div>
